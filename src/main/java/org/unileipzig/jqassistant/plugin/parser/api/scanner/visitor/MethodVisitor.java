...
public class MethodVisitor extends VoidVisitorAdapter<TypeDescriptor>{
	...
	public void visit(MethodDeclaration methodDeclaration, TypeDescriptor typeDescriptor) {
		...
		TypeDescriptor returnTypeDescriptor = typeResolver.resolveType(Utils.getQualifiedName(resolvedMethodDeclaration.getReturnType()));
		...
		for (int i = 0; i < parameters.size(); i++) {
			...
			TypeDescriptor parameterTypeDescriptor = typeResolver.resolveType(Utils.getQualifiedName(resolvedParameterDeclaration.getType()));
