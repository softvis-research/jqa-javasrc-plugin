...
public class AnnotationIT extends AbstractPluginIT {
    ...
    // */
    // @Test
    // public void annotatedField() throws IOException, NoSuchFieldException,
    ...
    // (f:Field)-[:ANNOTATED_BY]->(a:Java:Value:Annotation)-[:OF_TYPE]->(at:Type:Annotation)
    // RETURN f, a, at");
    // assertThat(testResult.getRows().size(), equalTo(1));
    // Map<String, Object> row = testResult.getRows().get(0);
    ...
    // (f:Field)-[:ANNOTATED_BY]->(a:Java:Value:Annotation)-[:HAS]->(value:Value)
    // RETURN value");
    // assertThat(testResult.getRows().size(), equalTo(1));
    // List<Object> values = testResult.getColumn("value");
    ...
    //
    // /**
    // * Verifies dependencies generated by default values of annotation
    // methods.
    // *
    // * @throws IOException
    // * If the test fails.
    // */
    // @Test
    // public void annotationDefaultValues() throws IOException,
    // NoSuchFieldException, NoSuchMethodException {
    // scanClasses(AnnotationWithDefaultValue.class);
    // store.beginTransaction();
    // assertThat(query("MATCH (t:Type:Annotation) RETURN t").getColumn("t"),
    // hasItem(typeDescriptor(AnnotationWithDefaultValue.class)));
    // assertThat(query("MATCH
    // (t:Type:Annotation)-[:DECLARES]->(m:Method)-[:HAS_DEFAULT]->(v:Value)
    // WHERE m.name='classValue' RETURN v").getColumn("v"),
    // hasItem(valueDescriptor(null, typeDescriptor(Number.class))));
    // assertThat(
    // query("MATCH
    // (t:Type:Annotation)-[:DECLARES]->(m:Method)-[:HAS_DEFAULT]->(v:Value)
    // WHERE m.name='enumerationValue' RETURN v").getColumn("v"),
    // hasItem(valueDescriptor(null, fieldDescriptor(DEFAULT))));
    // assertThat(query("MATCH
    // (t:Type:Annotation)-[:DECLARES]->(m:Method)-[:HAS_DEFAULT]->(v:Value)
    // WHERE m.name='primitiveValue' RETURN v").getColumn("v"),
    // hasItem(valueDescriptor(null, is(0d))));
    // assertThat(query("MATCH
    // (t:Type:Annotation)-[:DECLARES]->(m:Method)-[:HAS_DEFAULT]->(v:Value)
    // WHERE m.name='arrayValue' RETURN v").getColumn("v"),
    // hasItem(valueDescriptor(null, hasItem(valueDescriptor("[0]",
    // typeDescriptor(Integer.class))))));
    // assertThat(query("MATCH
    // (t:Type:Annotation)-[:DECLARES]->(m:Method)-[:HAS_DEFAULT]->(v:Value)
    // WHERE m.name='annotationValue' RETURN v").getColumn("v"),
    // hasItem(annotationValueDescriptor(NestedAnnotation.class,
    // hasItem(valueDescriptor("value", is("test"))))));
    // store.commitTransaction();
    // }
    //
    // /**
    ...
    // */
    // @Test
    // // @Ignore("Scanning the constructor of generic type fails if a parameter
    // annotation is present.")
    // public void innerClass() throws IOException, NoSuchFieldException,
    // NoSuchMethodException {
    // scanClasses(AnnotatedType.GenericInnerAnnotatedType.class,
    // Annotation.class);
    // store.beginTransaction();
    // TestResult testResult = query("MATCH
    // (c:Constructor)-[:HAS]->(:Parameter)-[:ANNOTATED_BY]->()-[:OF_TYPE]->(Type:Annotation)
    // RETURN c");
    // assertThat(testResult.getRows().size(), equalTo(1));
    // assertThat(testResult.getColumn("c"),
    // hasItem(constructorDescriptor(AnnotatedType.GenericInnerAnnotatedType.class,
    // AnnotatedType.class, Object.class)));
    // store.commitTransaction();
    // }
}
